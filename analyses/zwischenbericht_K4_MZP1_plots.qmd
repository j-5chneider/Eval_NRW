...
nrw_p <- nrw |>
  dplyr::select(kohorte, tpkcon_score, tpksit_score) |>
...
rm(nrw_p)
nrw_p <- nrw |>
  dplyr::select(kohorte, tpkpot_score,
                         tpksup_score,
                         tpksit_score_av,
                         tpksit_score_tb,
                         tpksit_score_aa) |>
...
```{r}
p1 <- ggplot(nrw_p, aes(#x=factor(kohorte, levels = c("4")), 
                        x=kohorte, 
                        y = palsma, 
...
nrw_p <- nrw |>
  dplyr::filter(kohorte %in% c("3", "4")) |>
  dplyr::filter(gruppe %in% c("sab")) |>
  dplyr::select(kohorte, gruppe, SExMPART, SExFAP, SExKAP) |>
  pivot_longer(3:5, names_to = "variables", values_to = "values")
