...
k4_laa_mzp1_clean <- k4_laa_mzp1 |>
  dplyr::select(seffic_01K4, bel_01K4, rfbe_01K4, 
                ...
                bel_01K4, aen_01K4, efft_01K4, entu_01K4, 
                palk_01K4, seffic_01K4, rfbe_01K4
                ) |>
...
k4_laa_mzp2_clean <- k4_laa_mzp2 |>
  dplyr::select(ubzeit1, ubzeit2, ub2_son1, ub2_son2,
                ...
                intal_tra1, intal_tra2, intal_tra3, zszf_2, ubqual_ftb1
                ) |>
...
k4_sab_mzp1_clean <- k4_sab_mzp1 |>
  dplyr::select(INNO_01K4, zszf_4_01K4:zszf_7_01K4, 
                # code_01_01K4,
                AUNLT_01K4, AUNL_01K4, INNO_01K4, zszf_2_01K4
                ) |>
  dplyr::rename_with(~ gsub("_01K4", "", .x, fixed = TRUE)) |> # Löschen von _01K4 in den Variablennamen
  dplyr::rename(c(uebr = AUNLT,
                  uebk = AUNL)) |>
...
k4_sab_mzp2_clean <- k4_sab_mzp2 |>
  dplyr::select(ubqual_ftb, ubqual_ebf, ubqual_ibno, ubqual_lob,
                ...
                # code_01,
                ub_au1, ub_nue1, 
                ubbeu_beu1, ubbeu_beu2, ubbeu_beu3, ubbeu_beu4, ubbeu_beu5, ubbeu_beu6
                ) |>
  # dplyr::rename(c(code = code_01)) |>
...
ggplot(nrw_p, aes(x = value, y = variable, fill = variable)) +
  geom_density_ridges(alpha = .8, bandwidth = 4, scale = 1.4) +
...
ggplot(nrw_p, aes(y=variable, x = value)) +
  ...
  scale_y_discrete(labels = c("Schule", "Seminar", "andere\nLAA", "Medien")) +
...
nrw_p <- nrw |>
  ...
  dplyr::mutate(value = factor(value,
                               ...
                               labels = c("trifft nicht zu",
                                          ...
                                          "trifft zu"))) |>
  pivot_wider(id_cols = c(id, kohorte, gruppe, mzp), names_from = "variable", values_from = "value") |>
  dplyr::select(ub1_au3a, ub1_au3b, ub1_au4a, ub1_au4b) |>
  dplyr::rename("fühle mich von SAB kontrolliert" = ub1_au3a,
                "fühle mich von SAB individuell unterstützt" = ub1_au4a, 
                "fühle mich von ALK kontrolliert" = ub1_au3b, 
                "fühle mich von ALK individuell unterstützt" = ub1_au4b)
...
nrw_p <- nrw |>
  ...
  dplyr::rename("ALK: Unterrichtsbesuche sind  Schaustunden, die mit dem alltäglichen Unterrichten nichts zu tun haben." = ub_8,
                "SAB: Die Unterrichtsbesuche lehren die Lehramts- anwärter_innen den ihnen bestmöglichen Unterricht vorzubereiten." = ub_nue1,
                "SAB: Unterrichtsbesuche sind ein angemessenes Abbild des Unterrichts einer Lehramts- anwärter_in" = ub_au1)

nrw_p |>
  gglikert(y_label_wrap = 32,
           symmetric = TRUE,
           add_labels = F,
           width = .6) +
  geom_vline(xintercept = 0, color = "lightgrey") +
...
nrw_p <- nrw |>
  dplyr::filter(variable %in% c("ubbeu_beu1", "ubbeu_beu2", "ubbeu_beu3", 
                                "ubbeu_beu4", "ubbeu_beu5", "ubbeu_beu6") & gruppe == "sab") |>
  dplyr::mutate(value = factor(value,
                               levels = c(1,2,3,4),
                               labels = c("trifft nicht zu",
                                          " ",
                                          ...
                                          "trifft zu"))) |>
  pivot_wider(id_cols = c(id, kohorte, gruppe, mzp), names_from = "variable", values_from = "value") |>
  dplyr::select(ubbeu_beu1, ubbeu_beu2, ubbeu_beu3, ubbeu_beu4, ubbeu_beu5, ubbeu_beu6) |>
  dplyr::rename("Ich achte darauf, die Lehramtsanwärter_innen nicht mehrmals in derselben Lerngruppe zu besuchen." = ubbeu_beu1, 
                "Mir ist es wichtig, dass die Lehramtsanwärter_innen fünf Unterrichtsbesuche pro Fach zeigen." = ubbeu_beu2, 
                "Für eine gute Bewertung erwarte ich eine genaue Umsetzung der vorherigen Planung." = ubbeu_beu3,
                "Ich benote jeden Unterrichtsbesuch." = ubbeu_beu4, 
                "Die in der Ordnung des Vorbereitungsdienstes und der Staatsprüfung (OVP) festgelegten Standards stehen bei der Bewertung der Unterrichtsbesuche im Vordergrund." = ubbeu_beu5, 
                "Der Referenzrahmen Schulqualität (RRSQ) steht bei der Bewertung der Unterrichtsbesuche im Vordergrund." = ubbeu_beu6)

nrw_p |>
  gglikert(y_label_wrap = 36,
           symmetric = TRUE,
           add_labels = F,
           width = .6) +
  geom_vline(xintercept = 0, color = "lightgrey") +
...
ggplot(nrw_p, aes(y=variable, x = value, group = gruppe, fill = gruppe, color = gruppe)) +
  ...
  geom_vline(xintercept = 2.5, color = "red", alpha = .4, size = 1.5) +
  scale_y_discrete(labels = c("Entwicklungs-\nbezogenes\nFeedback", 
                              "Fairness und\nTransparenz", 
                              "Individuelle\nBezugsnorm", 
                              "Lob")) +
  scale_x_continuous(breaks = c(1,2,3,4), labels = likert_labels, expand = c(0,0)) +
...
nrw_p <- nrw |>
  dplyr::filter(variable %in% c("ubqual_ftb1")) |>
  dplyr::mutate(value = factor(value,
                               levels = c(1,2,3,4),
                               labels = c("trifft nicht zu",
                                          " ",
                                          ...
                                          "trifft zu"))) |>
  dplyr::select(value) |>
  dplyr::rename("Ich konnte die Einschätzungen meiner Ausbilder_innen in den Nachbesprechungen immer nachvollziehen." = value)
...
**...waren fachlich gut ausgebildet.**
...
ggplot(nrw_p, aes(y=variable, x = value)) +
  stat_summary(geom = "col", fun = mean, alpha = .8, fill = "#34608d",
               width = .75) +
  stat_summary(linewidth = 1, geom = "errorbar", width = .25, color = "#34608d") +
  geom_vline(xintercept = 2.5, color = "red", alpha = .5, size = 1.5) +
  scale_y_discrete(labels = c("Pedagogical\nPractices",
                              "Professional\nRelationships")) +
  scale_x_continuous(breaks = c(1,2,3,4), labels = likert_labels) +
  coord_cartesian(xlim=c(1,4.2),
                  expand = F) +
  xlab("") +
  ylab("") +
...
ggplot(nrw_p, aes(y=variable_inhalt, x = value, 
                  group = gruppe, fill = gruppe, color = gruppe)) +
  stat_summary(geom = "col", fun = mean, alpha = .8, position = position_dodge()) +
  stat_summary(linewidth = 1, geom = "errorbar", 
               width = .25, position = position_dodge(width = .9)) +
  geom_vline(xintercept = 2.5, color = "red", alpha = .5, size = 1.5) +
  scale_y_discrete(labels = c("Konstruktivistisch",
                              "Transmissiv")) +
  scale_x_continuous(breaks = c(1,2,3,4), labels = likert_labels, expand = c(0,0)) +
  coord_cartesian(xlim=c(1,4.2),
                  expand = F) +
  ...
  xlab("") +
  ylab("") +
  labs(title = "Überzeugungen ",
       subtitle = "über das Lernen und Lehren",
       caption = "") +
  theme_minimal(base_size = 14) +
  theme(panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank(),
...
ggplot(nrw_p, aes(y=gruppe, x = value, fill = gruppe, color = gruppe)) +
  stat_summary(geom = "col", fun = mean, alpha = .8, 
               width = .8) +
  stat_summary(linewidth = 1, geom = "errorbar", width = .25) +
  geom_vline(xintercept = 2.5, color = "red", alpha = .5, size = 1.5) +
  scale_y_discrete(labels = c("LAA",
                              "ALK",
                              "SAB")) +
  scale_x_continuous(breaks = c(1,2,3,4), labels = likert_labels, expand = c(0,0)) +
...
**Meine Schule hat mit dem Zentrum für schulpraktische Lehrerausbildung gut zusammen gearbeitet.**

```{r}
nrw_p <- nrw |>
  dplyr::filter(variable %in% c("zszf_2")) |>
  dplyr::mutate(value = factor(value,
                               levels = c(1,2,3,4),
                               labels = c("trifft nicht zu",
                                          " ",
                                          "  ",
                                          "trifft zu"))) |>
  pivot_wider(id_cols = c(id, kohorte, variable, mzp), names_from = "gruppe", values_from = "value") |>
  dplyr::select(laa, sab, alk) |>
  dplyr::rename("LAA" = laa,
                "SAB" = sab,
                "ALK" = alk)
...
ggplot(nrw_p, aes(y=gruppe, x = value, fill = gruppe, group = gruppe, color = gruppe)) +
  stat_summary(geom = "col", fun = mean, alpha = .8, width = .8) +
  stat_summary(linewidth = 1, geom = "errorbar", 
               width = .25) +
  geom_vline(xintercept = 2.5, color = "red", alpha = .5, size = 1.5) +
  scale_y_discrete(labels = c("der Schule\n(ALK)",
...
```{r}
nrw_p <- nrw |>
  dplyr::filter(variable %in% c("soz_1", "soz_2", "soz_3") & gruppe == "alk") |>
  dplyr::mutate(value = factor(value,
                               levels = c(1,2,3,4),
                               labels = c("trifft nicht zu",
                                          " ",
...
## Eingangsvoraussetzungen LAA

```{r}
nrw_p <- nrw |>
  dplyr::filter(variable %in% c("bel", "aen", "efft", "entu", "palk", "seffic", "rfbe") &
                  gruppe == "laa" & mzp == "1")

ggplot(nrw_p, aes(y=variable, x = value)) +
  stat_summary(geom = "col", fun = mean, alpha = .8, fill = "#440d53") +
  stat_summary(linewidth = 1, geom = "errorbar", 
               width = .25, color = "#440d53") +
  geom_vline(xintercept = 2.5, color = "red", alpha = .5, size = 1.5) +
  scale_y_discrete(labels = c("Arbeitsengagement",
