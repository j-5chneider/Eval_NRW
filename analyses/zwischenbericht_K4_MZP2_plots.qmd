...
```{r}
nrw_p <- nrw |>
  dplyr::filter(variable %in% c("ub_7", "ub_8", "ub_1")) |>
...
nrw_p <- nrw |>
  dplyr::filter(variable %in% c("ub_au1")) |>
  dplyr::mutate(value = factor(value,
                               levels = c(1,2,3,4),
                               labels = c("trifft nicht zu",
                                          " ",
                                          ...
                                          "trifft zu"))) |>
  dplyr::select(value) |>
  dplyr::rename("Unterrichtsbesuche sind ein angemessenes Abbild des Unterrichts einer Lehramtsanwärter_in" = value)
