...
k4_laa_mzp2_clean <- k4_laa_mzp2 |>
  ...
  dplyr::mutate(id = 1:length(ubzeit1), # simply for pivot
                kohorte = as.factor("4"),    # Variable zur Unterscheidung der Kohorten
                gruppe = as.factor("laa"),   # Variable zur Unterscheidung der Gruppen
...
nrw_p <- nrw |>
  ...
  dplyr::rename("Ich empfand die Unterrichtsbesuche hilfreich." = ubnue_3, 
                "Die Unterrichtsbesuche haben mich gelehrt den mit bestmöglichem Unterricht vorzubereiten." = ubnue_4)
...
ggplot(nrw_p, aes(x=variable, y = value, group = gruppe, fill = gruppe)) +
...
ggplot(nrw_p, aes(x=value, y = variable, group = gruppe, fill = gruppe)) +
  stat_summary(geom = "col", fun = mean, alpha = .8, position = position_dodge()) +
  scale_y_discrete(labels = c("Bewertung erfolgt nach subjektiven\nMaßstäben der Prüfungskommission.",
                              "Entwicklung fließt nicht\nangemessen die Benotung des VD",
...
### Kooperation SAB und ALK: Seminargestaltung

```{r}
nrw_p <- nrw |>
  dplyr::filter(variable %in% c("INNO"))

ggplot(nrw_p, aes(x=gruppe, y = value)) +
  stat_summary(geom = "col", fun = mean, alpha = .8, 
               position = position_dodge(), fill = "#440d53") +
  scale_x_discrete(labels = c("ALK",
                              "SAB")) +
...
```{r}
nrw_p <- nrw |>
  dplyr::filter(variable %in% c("neuans_4", "koop_1", "koop_2", "neuans_2"))

ggplot(nrw_p, aes(x=value, y = variable)) +
  stat_summary(geom = "col", fun = mean, fill = "#440d53", alpha = .8) +
  scale_y_discrete(labels = c(
    "Zuschnitt der Ausbildungsprogramme\nauf Lehramt und Schule, auf Grundlage\ndes Kerncurriculums.",
    "Verzahnung schulpraktische Ausbildung\nmit Ausbildungsprogrammen durch ALK",
    "Absprache mit Ausbildungsschulen,\num Seminarinhalte an\nderen Arbeit anzubinden.",
    "Umsetzung neuer Ansätze\nzur Seminargestaltung erst, wenn\nim Ausbildungsprogramm verankert")) +
  scale_x_continuous(breaks = c(1,2,3,4), labels = likert_labels, expand = c(0,0)) +
  coord_cartesian(xlim=c(1,4.2),
                  expand = F) +
  xlab("") +
  ylab("") +
  labs(title = "Slbstbericht",
       subtitle = "der SAB",
       caption = "") +
  theme_minimal(base_size = 14) +
  theme(panel.grid.major.y = element_blank(),
